...
- **Cache Control Deployment v1.3**: Halaman utama dilengkapi dengan meta tag HTTP `Cache-Control` (`no-cache, no-store, must-revalidate`) serta cache-busting `?v=1.3.filemtime(...)` pada file CSS/JS untuk memastikan pengguna mendapatkan versi v1.3 aplikasi terbaru secara otomatis tanpa perlu *clear cache* manual.
